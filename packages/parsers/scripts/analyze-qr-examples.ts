...
async function main() {
	if (!existsSync(dir)) {
		console.log(
			"qr-examples/ not found. Add private local QR images to run live analysis.",
		);
		return;
	}
	...
	for (const file of files) {
		...
		try {
			...
			console.log(
				`  Total: ${invoice.totalAmount?.toFixed(2) ?? "não encontrado"}`,
			);
			console.log(
				`  Tributos aproximados: ${invoice.approximateTaxAmount?.toFixed(2) ?? "não encontrado"}`,
			);
			console.log(`  Percentual: ${tax.percentage.toFixed(1)}%`);
			console.log(`  Fonte: ${invoice.source}`);
		} catch (error) {
			console.log(
				`  Consulta: falhou (${error instanceof Error ? error.message : "erro desconhecido"})`,
			);
		}
	}
}
