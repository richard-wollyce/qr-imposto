...
export function decodeQrImage(buffer: Buffer): string | undefined {
	...
	const code = decodePreparedBitmap({
		data: rgba,
		width: bitmap.width,
...
function decodePreparedBitmap(bitmap: {
	data: Uint8ClampedArray;
	width: number;
	height: number;
}): string | undefined {
	return (
		jsQR(bitmap.data, bitmap.width, bitmap.height, {
			inversionAttempts: "attemptBoth",
		})?.data ?? decodeWithZxing(bitmap.data, bitmap.width, bitmap.height)
	);
}
