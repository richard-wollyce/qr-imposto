...
describe("parseNfceQrUrl", () => {
	it("accepts HTTPS NFC-e URLs and masks access keys", () => {
		...
		if (result.ok) {
			...
			expect(result.payload.sanitizedUrl).toContain(
				"352604********************************890123",
			);
		}
	});
	...
	it("rejects HTTP URLs before any NFC-e query leaves the device", () => {
		expect(
			parseNfceQrUrl(
				`http://www.nfce.fazenda.sp.gov.br/qrcode?p=${accessKey}|2|1|ABC`,
			),
		).toMatchObject({
			ok: false,
			reason: "unsupported_url",
...
describe("parseNfceFromText", () => {
	...
	it("keeps fetchNfcePublicPage HTTPS-only as a defense in depth", async () => {
		await expect(
			fetchNfcePublicPage(
				`http://www.nfce.fazenda.sp.gov.br/qrcode?p=${accessKey}`,
				async () => {
