...
export function parseNfceQrUrl(input: string): NfceQrParseResult {
	...
	if (url.protocol !== "https:") {
		return {
			...
			message:
				"Por seguranca, apenas URLs HTTPS de consulta de NFC-e sao aceitas.",
		};
	}
...
export async function analyzeNfceQrUrl(
	input: string,
	...
	if (!isSupportedMvpNfce(parsed.payload)) {
		return {
			...
			message:
				"Nesta primeira versao, o QR Imposto consulta NFC-e de SP. Outros estados entram nas proximas versoes.",
		};
	}
	...
	try {
		...
		return {
			...
			message:
				"Não foi possível consultar a NFC-e agora. Tente novamente em alguns instantes.",
		};
	}
...
export async function fetchNfcePublicPage(
	url: string,
	fetcher: FetchLike = fetch,
): Promise<string> {
...
export function isSupportedMvpNfce(
	payload: Pick<NfceQrPayload, "uf" | "host">,
): boolean {
...
export function parseNfceFromText(
	input: string,
	payload?: Partial<NfceQrPayload>,
): ParsedInvoice {
...
function parseXmlInvoice(
	...
	return {
		accessKey:
			payload?.accessKey ?? tagValue(input, "chNFe") ?? extractAccessKey(input),
...
function findCurrencyAfterLabels(
	text: string,
	labels: string[],
): number | undefined {
	...
	for (const label of labels) {
		...
		const value =
			parseCurrencyWithSymbol(window) ?? parseBrazilianCurrency(window);
...
function parseCurrencyWithSymbol(input: string): number | undefined {
	const match = input.match(
		/R\$\s*:?\s*\d{1,3}(?:\.\d{3})*(?:,\d{1,2})?|R\$\s*:?\s*\d+(?:,\d{1,2})?/i,
	);
...
function findIssuerName(text: string): string | undefined {
	...
	const labelMatch = compact.match(
		/(?:raz[aã]o social|emitente|estabelecimento)\s*:?\s*([^|]{4,80})/i,
	);
