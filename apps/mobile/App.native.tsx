...
import {
	confidenceLabel as formatConfidenceLabel,
	formatCurrency,
	formatPercentage,
	type ScanResult,
...
function AppContent() {
			...
			<View style={styles.permissionScreen}>
				...
				>
					...
					<Text style={styles.permissionTitle}>
						A câmera é necessária para ler o QR Code da NFC-e.
					</Text>
					<Text style={styles.permissionText}>
						A câmera é usada só para ler o QR Code. Após a leitura, o app
						consulta a URL HTTPS pública da NFC-e na SEFAZ para mostrar os
						valores da nota; o histórico fica local e não é salvo em nuvem.
					</Text>
					<Pressable
	...
	if (screen.status === "insights") {
		return (
			<InsightsScreen entries={historyEntries} onSelectTab={selectMainTab} />
		);
	}
				...
				>
					<View style={styles.scannerHeader}>
						...
						<Text style={styles.scannerSubtitle}>
							Aponte para o QR Code da nota fiscal de compra.
						</Text>
					</View>
						...
						{screen.status === "processing" ? (
							...
							<View style={styles.scannerHintGroup}>
								<Text style={styles.scannerHint}>
									Use uma NFC-e de mercado, loja, farmácia ou posto.
								</Text>
								<Pressable accessibilityRole="link" onPress={openPrivacyPolicy}>
									<Text style={styles.scannerPrivacyLinkText}>
	...
	const tabItems = [
		{
			...
			label: "Scanner",
			icon: (color: string) => (
				<ScanLine color={color} size={21} strokeWidth={2.4} />
			),
		},
		{
			id: "history" as const,
			label: "Histórico",
			icon: (color: string) => (
				<HistoryIcon color={color} size={21} strokeWidth={2.4} />
			),
		},
		{
			...
			label: "Insights",
			icon: (color: string) => (
				<ChartColumn color={color} size={21} strokeWidth={2.4} />
			),
		},
	];
				...
				return (
					<Pressable
						...
						style={[
							...
							isActive
								? isDark
									? styles.bottomTabButtonActiveDark
									: styles.bottomTabButtonActiveLight
								: null,
...
function ResultScreen({
	...
	const percent = useMemo(
		() => formatPercentage(result.computation.percentage),
		[result.computation.percentage],
	);
	const shareCardPayload = useMemo(
		() => buildScanShareCardPayload(result),
		[result],
	);
	const issuer =
		result.invoice.issuerName ?? "Estabelecimento não identificado";
		...
		<View style={styles.resultScreen}>
			...
			>
				...
				<View style={styles.resultHeroCard}>
					...
					<Text
						style={styles.resultHeroAmount}
						numberOfLines={1}
						adjustsFontSizeToFit
					>
						{formatCurrency(result.computation.approximateTaxAmount)}
					</Text>
					<Text style={styles.resultHeroDetail}>
						{percent} do valor pago nesta compra.
					</Text>
				</View>
				...
				<View style={styles.metricGrid}>
					<Metric
						label="Compra"
						value={formatCurrency(result.computation.totalAmount)}
					/>
...
function ShareCardPanel({ payload }: { payload: ShareCardPayload }) {
	...
	const primaryLabel =
		shareState.status === "sharing" && shareState.action === "system"
			? "Gerando card..."
			: "Compartilhar card";
	...
	const shareCard = useCallback(async () => {
		...
		try {
			const image = await captureShareCardImage(
				cardRef as ShareCardCaptureRef,
				payload.fileName,
			);
			...
			if (didShare) {
				setShareState({
					status: "success",
					message: "Opções de compartilhamento abertas.",
				});
				return;
			}
			...
			setShareState({
				status: "error",
				message: "Compartilhamento indisponível neste dispositivo.",
			});
		} catch (error) {
	...
	return (
		<View style={styles.sharePanel}>
			<ShareCardPreview
				payload={payload}
				cardRef={cardRef as ShareCardCaptureRef}
			/>
		...
		<View style={styles.sharePreviewFrame}>
			<View ref={cardRef} collapsable={false} style={styles.shareCard}>
				...
				<View style={styles.shareCardMain}>
					{payload.primaryIntro ? (
						<Text style={styles.shareCardIntro}>{payload.primaryIntro}</Text>
					) : null}
					<Text
						style={styles.shareCardAmount}
						numberOfLines={1}
						adjustsFontSizeToFit
					>
				...
				<View style={styles.shareCardStats}>
					{payload.statRows.map((row) => (
						<View key={row.label} style={styles.shareCardStatRow}>
							<Text
								style={styles.shareCardStatLabel}
								numberOfLines={1}
								adjustsFontSizeToFit
							>
								{row.label}
							...
							<Text
								style={styles.shareCardStatValue}
								numberOfLines={1}
								adjustsFontSizeToFit
							>
								{row.value}
				...
				{entries.length > 0 ? (
					...
					<View style={styles.emptyHistory}>
						...
						<Text style={styles.historyText}>
							Quando uma NFC-e de SP for lida com sucesso, ela aparece aqui.
						</Text>
					</View>
				)}
...
function InsightsScreen({
	...
	const summaries = useMemo(
		() => summarizeScanHistory(entries, now),
		[entries, now],
	);
	const weeklySeries = useMemo(
		() => buildWeeklyTaxSeries(entries, now),
		[entries, now],
	);
	const monthlySeries = useMemo(
		() => buildMonthlyTaxSeries(entries, now),
		[entries, now],
	);
	const yearlySeries = useMemo(
		() => buildYearlyTaxSeries(entries, now),
		[entries, now],
	);
	const [selectedSummaryPayload, setSelectedSummaryPayload] =
		useState<ShareCardPayload | null>(null);
	const openSummaryCard = useCallback(
		(periodKey: SharePeriodKey, summary: HistorySummary) => {
			setSelectedSummaryPayload(
				buildSummaryShareCardPayload(periodKey, summary),
			);
		},
		[],
	);
		...
		<View style={styles.resultScreen}>
			...
			>
				...
				<TodayTotalCard
					...
					onPress={() => openSummaryCard("today", summaries.today)}
				/>
				...
				<View style={styles.periodGrid}>
					<PeriodInsightCard
						...
						onPress={() => openSummaryCard("week", summaries.week)}
					/>
					<PeriodInsightCard
						label="Mês"
						summary={summaries.month}
						onPress={() => openSummaryCard("month", summaries.month)}
					/>
					<PeriodInsightCard
						label="Ano"
						summary={summaries.year}
						onPress={() => openSummaryCard("year", summaries.year)}
					/>
				</View>

				<TaxEvolutionCard
					title="Evolução dos tributos na semana"
					subtitle="Dia a dia da semana atual"
					series={weeklySeries}
				/>
				<TaxEvolutionCard
					title="Evolução dos tributos no mês"
					subtitle="Dia a dia do mês atual"
					series={monthlySeries}
				/>
				<TaxEvolutionCard
					title="Evolução dos tributos no ano"
					subtitle="Mês a mês do ano atual"
					series={yearlySeries}
				/>
				...
				<View style={styles.methodologyCard}>
					...
					<Text style={styles.methodologyBody}>
						Os valores são aproximados e calculados a partir das NFC-es lidas
						localmente. O histórico não é enviado para servidor e pode variar
						conforme os dados públicos disponíveis em cada nota.
					</Text>
				</View>
			</ScrollView>
			<BottomTabBar
					...
					<View style={styles.shareModalSheet}>
						<View style={styles.shareModalHeader}>
							...
							<Pressable
								...
								onPress={() => setSelectedSummaryPayload(null)}
							>
								<Text style={styles.secondaryButtonText}>Fechar</Text>
							</Pressable>
						</View>
						<ScrollView contentContainerStyle={styles.shareModalContent}>
							{selectedSummaryPayload ? (
								<ShareCardPanel payload={selectedSummaryPayload} />
							) : null}
						</ScrollView>
					</View>
				</View>
...
function ScreenHeader({
	...
	return (
		<View style={styles.screenHeader}>
			...
			<Pressable
				accessibilityRole="button"
				accessibilityLabel="Voltar ao scanner"
	...
	return (
		<View style={styles.historyItem}>
			<View style={styles.historyItemHeader}>
				...
				<Pressable
					accessibilityRole="button"
					accessibilityLabel="Remover leitura"
			...
			<View style={styles.historyItemMetrics}>
				...
				<HistoryMetric
					label="Impostos"
					value={formatCurrency(entry.approximateTaxAmount)}
					tone="tax"
				/>
				<HistoryMetric
					label="Peso"
					value={formatPercentage(entry.percentage)}
				/>
			</View>
	...
	return (
		...
		>
			...
			<Text style={styles.todayCardDetail}>
				{summary.count} {countLabel} • {formatPercentage(summary.percentage)} do
				valor pago
			</Text>
		</Pressable>
	);
...
function TaxEvolutionCard({
	...
	const totalTax = series.reduce(
		(total, point) => total + point.approximateTaxAmount,
		0,
	);
...
function TaxLineChart({ series }: { series: HistorySeriesPoint[] }) {
	...
	return (
		<View style={styles.chartBox}>
				...
				{chartPoints
					...
					.map((point) => (
						<Circle
							...
							cy={point.y}
							r={4}
							fill={point.value > 0 ? "#1349EC" : "#AEB8CB"}
						/>
					))}
			</Svg>
			...
			{!hasValues ? (
				<Text style={styles.chartEmptyText}>Sem leituras neste período.</Text>
			) : null}
		</View>
	);
}
...
function buildChartPoints(series: HistorySeriesPoint[]): ChartPoint[] {
	...
	const maxValue = Math.max(
		...series.map((point) => point.approximateTaxAmount),
		1,
	);
