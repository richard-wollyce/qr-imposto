...
export async function loadScanHistory(): Promise<ScanHistoryEntry[]> {
	...
	try {
		...
		return Array.isArray(parsed)
			? parsed.filter(isScanHistoryEntry).slice(0, MAX_HISTORY_ENTRIES)
			: [];
	} catch {
		return [];
	}
...
export async function saveScanResult(
	...
	if (entry.dedupeKey) {
		const duplicate = entries.find(
			(existingEntry) => existingEntry.dedupeKey === entry.dedupeKey,
		);
...
export async function removeScanHistoryEntry(
	id: string,
...
export function summarizeScanHistory(
	entries: ScanHistoryEntry[],
	now = new Date(),
): HistorySummaries {
	...
	return {
		today: summarize(
			entries.filter((entry) => isOnOrAfter(entry.scannedAt, todayStart)),
		),
		week: summarize(
			entries.filter((entry) => isOnOrAfter(entry.scannedAt, weekStart)),
		),
		month: summarize(
			entries.filter((entry) => isOnOrAfter(entry.scannedAt, monthStart)),
		),
		year: summarize(
			entries.filter((entry) => isOnOrAfter(entry.scannedAt, yearStart)),
		),
		all: summarize(entries),
	};
}

export function buildWeeklyTaxSeries(
	entries: ScanHistoryEntry[],
	now = new Date(),
): HistorySeriesPoint[] {
	const weekStart = startOfWeek(now);
...
export function buildMonthlyTaxSeries(
	entries: ScanHistoryEntry[],
	now = new Date(),
): HistorySeriesPoint[] {
	const monthStart = startOfMonth(now);
...
export function buildYearlyTaxSeries(
	entries: ScanHistoryEntry[],
	now = new Date(),
): HistorySeriesPoint[] {
...
function summarize(entries: ScanHistoryEntry[]): HistorySummary {
	...
	const approximateTaxAmount = entries.reduce(
		(total, entry) => total + entry.approximateTaxAmount,
		0,
	);
...
function buildSeriesPoint(
	...
	return {
		...
		endDate,
		...summarize(
			entries.filter((entry) => isInRange(entry.scannedAt, startDate, endDate)),
		),
	};
}
...
function isInRange(input: string, startDate: Date, endDate: Date): boolean {
	const parsed = new Date(input);
	return (
		Number.isFinite(parsed.getTime()) && parsed >= startDate && parsed < endDate
	);
}
