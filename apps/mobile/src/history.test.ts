...
vi.mock("expo-crypto", () => ({
	...
	digestStringAsync: vi.fn(
		async (_algorithm: string, input: string) => `sha256-${input}`,
	),
}));
...
describe("scan history", () => {
	...
	it("summarizes the current day without including previous readings", () => {
		...
		const entries = [
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 20, 9, 0, 0).toISOString(),
				totalAmount: 100,
				approximateTaxAmount: 20,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 19, 21, 0, 0).toISOString(),
				totalAmount: 80,
				approximateTaxAmount: 8,
			}),
		];
	...
	it("builds a weekly tax series with seven points and zero-value days", () => {
		...
		const entries = [
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 20, 9, 0, 0).toISOString(),
				totalAmount: 200,
				approximateTaxAmount: 50,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 22, 10, 0, 0).toISOString(),
				totalAmount: 100,
				approximateTaxAmount: 10,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 27, 9, 0, 0).toISOString(),
				totalAmount: 999,
				approximateTaxAmount: 999,
			}),
		];
	...
	it("builds a monthly tax series for every day in the current month", () => {
		...
		const entries = [
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 1, 9, 0, 0).toISOString(),
				totalAmount: 50,
				approximateTaxAmount: 5,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2026, 3, 30, 18, 0, 0).toISOString(),
				totalAmount: 120,
				approximateTaxAmount: 30,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2026, 2, 31, 23, 0, 0).toISOString(),
				totalAmount: 999,
				approximateTaxAmount: 999,
			}),
		];
		...
		expect(series[29]).toMatchObject({
			key: "2026-04-30",
			label: "30",
			count: 1,
			approximateTaxAmount: 30,
	...
	it("builds a yearly tax series with twelve monthly points", () => {
		...
		const entries = [
			makeHistoryEntry({
				scannedAt: new Date(2026, 0, 10, 9, 0, 0).toISOString(),
				totalAmount: 90,
				approximateTaxAmount: 9,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2026, 11, 25, 9, 0, 0).toISOString(),
				totalAmount: 300,
				approximateTaxAmount: 60,
			}),
			makeHistoryEntry({
				scannedAt: new Date(2025, 11, 25, 9, 0, 0).toISOString(),
				totalAmount: 999,
				approximateTaxAmount: 999,
			}),
		];
...
function makeScanResult(accessKey: string): Extract<ScanResult, { ok: true }> {
	return {
		...
		insight: {
			...
			impact:
				"A cada R$ 100,00 nesse padrão de compra, cerca de R$ 20,00 aparecem como tributos aproximados.",
...
function makeHistoryEntry(
	...
	return {
		...
		scannedAt:
			overrides.scannedAt ?? new Date(2026, 3, 20, 12, 0, 0).toISOString(),
		issuerName: overrides.issuerName ?? "Mercado Exemplo",
		totalAmount,
		approximateTaxAmount,
		percentage:
			totalAmount > 0 ? (approximateTaxAmount / totalAmount) * 100 : 0,
