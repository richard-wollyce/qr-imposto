...
describe("share card payload", () => {
	it("formats the individual scan card without sensitive NFC-e data", () => {
		const payload = buildScanShareCardPayload(
			makeSensitiveScanResult(),
		...
		expect(serialized).not.toContain(
			"35260412345678000195650010000001234567890123",
		);
	...
	it.each<[SharePeriodKey, string, string]>([
		...
		expect(payload.shareText).toContain(
			`${eyebrow}, paguei R$ 40,00 em tributos aproximados`,
		);
	...
	it("uses singular copy for a one-note summary card", () => {
		...
		expect(payload.shareText).toContain(
			"Hoje, paguei R$ 18,40 em tributos aproximados na minha compra",
		);
	...
	it("does not break summary cards with zero notes", () => {
		...
		expect(payload.statRows).toContainEqual({
			label: "VALOR DAS COMPRAS",
			value: "R$ 0,00",
		});
		expect(payload.statRows).toContainEqual({
			label: "NOTAS DE COMPRA NFC-E",
			value: "0 notas escaneadas",
		});
		expect(payload.statRows).toContainEqual({
			label: "PORCENTAGEM DE IMPOSTO",
			value: "0% do valor das compras",
		});
	...
	it("builds a stable PNG filename", () => {
		expect(
			buildShareCardFileName(new Date("2026-04-20T12:00:00.000Z"), "hoje"),
		).toBe("qr-imposto-hoje-2026-04-20.png");
	});
});
